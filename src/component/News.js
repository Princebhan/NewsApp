 ...
 async componentDidMount(){
  ...
  render() {
    return (
      ...
      <h1 className='text-center' style={{margin:'35px 0px'}}>24*7 Top Headlines</h1>
      {this.state.loading && <Spinner />}
          ...
          {!this.state.loading && this.state.articles.map((element)=>{
             return <div className="col-md-4" key={element.url}>
              ...
              <NewsIteam  newUrl={element.url} title={element.title?element.title.slice(0, 80):""} description={element.description?element.description.slice(0, 77):""} imageurl={element.urlToImage}
        ...
        <div className="container d-flex justify-content-between">
          ...
          <button type='button' disabled={this.state.page +1 >Math.ceil(this.statetotalResults/15)} className='btn btn-dark' onClick={this.handlenextclick}>Next&rarr;</button>
        </div>
